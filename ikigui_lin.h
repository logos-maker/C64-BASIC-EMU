...
void ikigui_window_get_events(ikigui_window *mywin){
	// values for recognicing changes in mousemovements and mouse buttons.
		...
		/*
			...
			switch(mywin->key){
				case XK_Left:	mywin->text[0] = 24; printf("Left"); break;
				case XK_Right:	mywin->text[0] = 25; printf("Right"); break;
				case XK_Up:	mywin->text[0] = 26; printf("Up"); break;
				case XK_Down:	mywin->text[0] = 27; printf("Down"); break;
			}
	        }
		*/
                switch(mywin->event.type){
			case KeyPress: {
				// This new one to deal with the emulator is from 20250725 ->
				int len = XLookupString(&mywin->event.xkey, mywin->text, 255, &mywin->key, 0);

				// Ignore modifier keys / Ignore dead keys (like " on some layouts) to avoid duplicates
				if (mywin->key == XK_Shift_L || mywin->key == XK_Shift_R ||
				    mywin->key == XK_Control_L || mywin->key == XK_Control_R ||
				    mywin->key == XK_Alt_L || mywin->key == XK_Alt_R) {
				    mywin->key = 0;
				    break;
				}

				if (len == 1) { } // printable char in mywin->text[0]
				else {
					switch (mywin->key) { // handle special keys or ignore dead keys
						case XK_Left:  mywin->text[0] = 24; break;
						case XK_Right: mywin->text[0] = 25; break;
						case XK_Up:    mywin->text[0] = 26; break;
						case XK_Down:  mywin->text[0] = 27; break;
						default: if (mywin->key >= XK_dead_grave && mywin->key <= XK_dead_semivoiced_sound) { mywin->key = 0; }  break; // discard and ignore dead keys
					}
				}
			}
			/* the old
