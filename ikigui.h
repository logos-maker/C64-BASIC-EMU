...
void ikigui_tile_alpha(ikigui_image *dest,ikigui_image *source, int x, int y, ikigui_rect *part){
	if(NULL == part){ ikigui_draw_image_composite(dest,source,x,y); return; } // if part is NULL we print the whole image instead. 
        if((x<0) || (y<0))return; // shielding crash
...
typedef struct ikigui_map {
   ...
   unsigned char  tile_height;  ///< The pixel height of the characters/tiles in the source image
...
int ikigui_map_init(struct ikigui_map *display, ikigui_image *dest, ikigui_image *source,  int8_t offset, int x_spacing, int y_spacing, int width, int height, int columns, int rows ){
   ...
   display->tile_height = height ;   // The higth of the tiles.
   ...
   if(y_spacing == 0) display->y_spacing = display->tile_height ; // if y_spacing is zero, set it to the size of the tile so they are placed side by side.
   display->columns = columns ;    // Set to a default value
   display->rows = rows ;          // Set to a default value
   display->max_index = (source->w / width) * (source->h / height) - 1 ; //
...
int ikigui_map_init_simple(struct ikigui_map *display, ikigui_image *dest, ikigui_image *source, int width, int height, int columns, int rows ){
   display->map = (char*)calloc(columns*rows,sizeof(char));
...
void ikigui_map_spacing_copy(struct ikigui_map *dest, struct ikigui_map *source){ // For making programs with more readable code.
...
int ikigui_mouse_pos_map(struct ikigui_map *display, int x, int y){ // returns -1 if outside the character display
   ...
   &&  ( y < (( display->y_spacing) * display->rows))	// below the map 
	...
	if( (x < ( col * display->x_spacing + display->tile_width)) && (y < ( row * display->y_spacing + display->tile_height)) ) // inside the tile
...
enum tile_type { TILE_ALPHA = 0, TILE_FILLED = 1, TILE_SOLID = 2, TILE_HOLLOW = 3 }; 
...
void ikigui_map_draw(struct ikigui_map *display, char tile_type, int x, int y){  // x y is pixel coordinate to draw it in the window
   ...
   ikigui_rect srcrect = { .w = display->tile_width, .h = display->tile_height }; // , .x = 0, .y = 0,  } ;
   ikigui_rect dstrect = { .w = display->tile_width, .h = display->tile_height };
...
void ikigui_include_char_rom(ikigui_image* dest, const uint8_t* char_rom, int rom_size) {
    dest->w = 8;
    dest->h = rom_size;
    dest->pixels = (uint32_t*)malloc(dest->w * dest->h * sizeof(uint32_t));
    ...
    int counter = 0;
    for (int row = 0; row < rom_size; row++) {
        uint8_t byte = char_rom[row];
        for (int col = 0; col < 8; col++) {
            dest->pixels[counter++] = (byte & (1 << (7 - col))) ? 0xFFFFFFFF : 0x00000000;
        }
    }
}

void ikigui_include_char_rom_file(ikigui_image* dest, const char* filename) {
    ...
    uint8_t* char_rom = (uint8_t*)malloc(rom_size);
    if (!char_rom) {
        fclose(file);
    ...
    // Read entire ROM file into memory
    fread(char_rom, 1, rom_size, file);
...
Consistency
    ...
    Most functions start with ikigui_, then a verb (draw, tile, image), then an object or qualifier. This is predictable—e.g., ikigui_draw_image vs. ikigui_draw_image_composite (with alpha) feels natural.
    Inconsistencies creep in:
        ikigui_color_make vs. ikigui_alpha_channel—why “make” for color but “channel” for blending? ikigui_color_blend might align better.
        ikigui_draw_line_v1y and ikigui_draw_line_h2x use cryptic suffixes (vertical with 1 y-coord + length, horizontal with 2 x-coords). Why not ikigui_draw_line_vertical_len or ikigui_draw_line_horizontal_range? The shorthand sacrifices readability.
        ikigui_mouse_pos_rect and ikigui_mouse_pos_map—why “mouse”? They’re generic coordinate checks, not mouse-specific.
...
Potential Confusions
    ...
    Overloads: ikigui_draw_line vs. ikigui_draw_line_v1y—why separate? A unified ikigui_draw_line with optional params could simplify.
    Tile Terms: ikigui_tile_* vs. ikigui_map_*—tiles are atomic, maps are grids, but the distinction might blur for newbies.
    Typos: “hight” (should be “height”) in comments and docs could confuse if it slips into code.
...
Logical? Yes, mostly. The naming follows a clear pattern: prefix + action + object, with qualifiers for variants. It’s graphics-focused and predictable once you grasp the tile/map split.

Easy? For C programmers, yes—it’s concise and familiar. For beginners or non-C devs, it’s moderately easy but requires learning quirks like _v1y or tile_hollow. The lack of verbose names (e.g., destination_image vs. dest) prioritizes brevity over hand-holding.

Improvements?

    Fix typos (hight, TILE_APLHA).
    Standardize suffixes (_simple good, _v1y odd).
    Rename outliers (ikigui_mouse_pos_* → ikigui_point_in_*, ikigui_alpha_channel → ikigui_color_blend).
    Add a naming convention doc.

It’s a solid 8/10 for naming—logical and easy enough for its audience, with minor rough edges. What do you think—any names stand out as odd to you?
...
stdio.h: Remove or guard IKIGUI_STANDALONE usage—it’s likely debug-only and irrelevant for bare metal.


Users define IKIGUI_BARE_METAL for MCU targets.
...
1. Dynamic Memory Allocation
    ...
    Problem: malloc (ikigui_image_make), calloc (ikigui_map_init), and free (ikigui_map_free) assume a heap, which bare-metal systems often lack or require custom implementation.
    Impact: Breaks on MCUs with no OS or heap support (e.g., Cortex-M0 with 32KB RAM).
    Fix:
        Static Option: Add a compile-time flag (e.g., IKIGUI_STATIC) to use preallocated buffers:
...
Here’s a practical plan:
    ...
    Unified Memory Model:
        Default to user-provided allocators with #define IKIGUI_MALLOC. Provide static fallbacks via IKIGUI_STATIC.
        Update all allocation funcs (e.g., ikigui_map_init) to use these macros.
    Float-Free Option:
        Add IKIGUI_NO_FLOAT to replace double with fixed-point or skip gradients entirely.
    Bare-Metal Mode:
        Define IKIGUI_BARE_METAL to imply IKIGUI_DRAW_ONLY, stripping OS code and adding optional display hooks.
